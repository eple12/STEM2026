    ...
    while run:
        ...
        while(i < len(cars)):
            ...
            if t>10 and (car.detectCollision(road) or y > world.getBestCarPos()[1] + BAD_GENOME_TRESHOLD or y>y_old or car.vel < 0.1): #il t serve a evitare di eliminare macchine nei primi tot frame (nei primi frame getCollision() restituisce sempre true)
                ...
                is_risky = input[BRAKE_RISK_SENSOR_INDEX] > BRAKE_RISK_DISTANCE and (car.vel / MAX_VEL) > BRAKE_RISK_SPEED
                is_braking = decodeCommand(car.commands, BRAKE)
                if is_risky:
                    ge[i].fitness += BRAKE_USE_REWARD if is_braking else -BRAKE_MISS_PENALTY
